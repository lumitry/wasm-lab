...
- [ ] submit the plan and slideshow to mycourses on the weekend
- [ ] idea: flappy bird but with lots of constants for tweaking the gameplay (speed, gravity, bird size, pipe spacing & quantity, etc.)
- [ ] compare computation of a factorial of a large number in JS natively vs a very efficient WASM implementation (in C?). maybe also doing it multiple times in a row to show how the overhead of WASM is amortized over multiple calls.
